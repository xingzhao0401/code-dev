...
RUN apk add --no-cache vim bash go python python-dev py2-pip python3 python3-dev nodejs npm dumb-init openssl musl-dev  &&\
   ...
   rm -f /workdir/code-server-*.tar.gz /workdir/ms-go.vsix /workdir/ms-python.vsix ms-Vetur.vsix ms-Vue2Snippets.vsix ms-AutoCloseTag.vsix ms-AutoRenameTag.vsix
